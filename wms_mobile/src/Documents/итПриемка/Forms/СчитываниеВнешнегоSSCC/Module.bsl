...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    ПомарочныйУчет=ОбъектДанных.ПомарочныйУчет;
КонецПроцедуры
...
Процедура ПрименитьКДокументу()
	Если ЗначениеЗаполнено(СчитанныйSSCC) тогда
		...
		КонецЕсли;	
		Если Отказ тогда
			Сообщить("Данные SSCC не числится в помарочном учете");
			Возврат
		КонецЕсли;
		...
		ОбработчкаПолученияДанныхШтрихКода(СчитанныйSSCC);	
...
Процедура ПрисвоитьВнешнийSSCCНаСервере(SSCC,Отказ)
	...
	СтруктураДанных.Вставить("ТипОбработкиДанных","ВнешнийSSCCПриемкаКакЕсть");
...
Процедура ОбработчкаПолученияДанныхШтрихКода(ШтрихКод)
	...
	Если ИнициализацияЗадачиНаСервере(СтруктураОтвета[ПредставлениеТипаЗадачи]) тогда
		Если ПомарочныйУчет тогда
			СтруктураОтвета[ПредставлениеТипаЗадачи].Вставить("СчитываниеМарок");
		КонецЕсли;
		ОткрытьФорму("Документ.итПриемка.Форма.ФормаЗадачиДокумента",СтруктураОтвета[ПредставлениеТипаЗадачи],ЭтаФорма.ВладелецФормы);
