...
Процедура ЗаполнитьЗначенияФормы()
	...
	Запрос.Текст = 
		...
		|	ШтрихКодыДляЯчейкиОтправитель.ШтрихКод КАК ШтрихКодЯчейкиОтправитель,
		|	ШтрихКодыДляЯчейкиПолучатель.ШтрихКод КАК ШтрихКодЯчейкиПолучатель,
		|	ИтЗадачиНаТСД.Состояние КАК Состояние
		|ИЗ
		|	РегистрСведений.ИтЗадачиНаТСД КАК ИтЗадачиНаТСД
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.итРазмещение.ШтрихКодыЯчеекДокумента КАК ШтрихКодыДляЯчейкиПолучатель
		|		ПО ИтЗадачиНаТСД.ДокументОснование = ШтрихКодыДляЯчейкиПолучатель.Ссылка
		|			И ИтЗадачиНаТСД.ЯчейкаПолучатель = ШтрихКодыДляЯчейкиПолучатель.Ячейка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.итРазмещение.ШтрихКодыЯчеекДокумента КАК ШтрихКодыДляЯчейкиОтправитель
		|		ПО ИтЗадачиНаТСД.ДокументОснование = ШтрихКодыДляЯчейкиОтправитель.Ссылка
		|			И ИтЗадачиНаТСД.ЯчейкаОтправитель = ШтрихКодыДляЯчейкиОтправитель.Ячейка
		|ГДЕ
...
//СлужебныеФункцииИПроцедурыКлиентСервер.ДобавитьВЛог(ИдЗадачи,"#РазмещениеФормаЗадачи Сканирование");
...
Если Режим="РежимСканированиеПаллетыНачало" тогда
	Если СокрЛП(Параметр)=СокрЛП(ИдентификаторУпаковки) тогда
		//СлужебныеФункцииИПроцедурыКлиентСервер.ДобавитьВЛог(ИдЗадачи,"+#РазмещениеФормаЗадачи ИнициализацияЗадачиНаСервере РежимСканированиеПаллетыНачало");
		ИнициализацияЗадачиНаСервере(Режим);
		//СлужебныеФункцииИПроцедурыКлиентСервер.ДобавитьВЛог(ИдЗадачи,"-#РазмещениеФормаЗадачи ИнициализацияЗадачиНаСервере РежимСканированиеПаллетыНачало");
		ЗаполнитьЗначенияФормы();
		ДоступностьВидимостьЭлементовКлиент();
...
Если Режим="РежимСканированияЯчейкиПолучатель" тогда
	...
	Если НРег(СокрЛП(Параметр))=НРег(СокрЛП(ШтрихКодЯчейкиПолучатель)) тогда
		...
		//СлужебныеФункцииИПроцедурыКлиентСервер.ДобавитьВЛог(ИдЗадачи,"+#РазмещениеФормаЗадачи ИнициализацияЗадачиНаСервере РежимСканированияЯчейкиПолучатель");
...
Возврат ОбработчикДанныхОбщиеФункцииИПроцедуры.ИнициализацияЗадачиНаСервере(СтруктураДанных,"Задача",Перечисления.итWMSСостоянияЗадачТСД.Выполнена);
