...
&НаКлиенте
Процедура ШкКакКороб(Команда)
	Если ШККакКороб Тогда
		ШККакКороб=Ложь;
		//@skip-check new-color
		Элементы.ШкКакКороб.ЦветФона=новый Цвет(255,255,255);
	иначе
		ШККакКороб=Истина;
		ШкКакПаллета=Ложь;
		//@skip-check new-color
		Элементы.ШкКакКороб.ЦветФона=новый Цвет(167,255,73);	
		//@skip-check new-color
		Элементы.ШкКакПаллета.ЦветФона=новый Цвет(255,255,255);
		ШкКакПаллета=Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ШкКакПаллета(Команда)
	Если ШкКакПаллета Тогда
		ШкКакПаллета=Ложь;
		//@skip-check new-color
		Элементы.ШкКакПаллета.ЦветФона=новый Цвет(255,255,255);
	иначе
		...
		Элементы.ШкКакКороб.ЦветФона=новый Цвет(255,255,255);	
		//@skip-check new-color
		Элементы.ШкКакПаллета.ЦветФона=новый Цвет(167,255,73);
	КонецЕсли;
КонецПроцедуры
